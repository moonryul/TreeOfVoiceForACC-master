       ...
       { // the endBytes are received, and the data received is considered valid.
         ...
         // check if the data received can be captured by loop() 
          if ( m_process_it == false ) 
           ...
           } // (m_process_it == false ) the buffer can be read by loop() because loop() is free
           ...
           } // ( m_process_it == true ) the buffer cannot be read by loop(), because it is busy with reading the previous buffer
