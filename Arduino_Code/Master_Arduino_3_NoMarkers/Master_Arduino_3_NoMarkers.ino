...
const int m_totalNumOfPixels = NumPixe1s1 +  NumPixe1s2 +  NumPixe1s3 +  NumPixe1s4;
...
void readFrameWithoutMarkers()
{
  ...
  void showNewFrame() {
    if ( m_newFrameHasArrived == true ) {
      ...
      sendLEDBytesToSlavesWithoutMarkers(m_totalReceiveBuffer,  m_totalByteSize );
      
      //sendLEDBytesToSlavesWithMarkers(m_totalReceiveBuffer,  m_totalByteSize );
  ...
  void  sendLEDBytesToSlavesWithMarkers( byte * totalReceiveBuffer, int totalByteSize )
  {
    // use deviceA
    ...
    delay(2);
  } //  sendLEDBytesToSlavesWithoutMarkers(totalReceiveBuffer,  m_totalByteSize )
