...
namespace SimpleFileBrowser
{
	public class FileBrowser : MonoBehaviour, IListViewAdapter
	{
        ...
        static GameObject m_commHub;

        static GameObject m_fileBrowserCanvasObj;
        

        Canvas m_browserCanvas;
...
#pragma warning restore 0649
        ...
        // Static Fields and Properties are initialled when the class is loaded
		...
		private static FileBrowser Instance
		{
			get
			{
				if( m_instance == null )
				{
                   ...
                   // m_fileBrowserCanvasObj = Instantiate(Resources.Load<GameObject>("SimpleFileBrowserCanvas")) ;
                  //  m_commHub = GameObject.FindGameObjectWithTag("CommHub");

                  //  m_fileBrowserCanvasObj.transform.SetParent(m_commHub.transform, false);

                 //   m_instance = m_fileBrowserCanvasObj.GetComponent<FileBrowser>();
...
#pragma warning disable 0649
        ...
        // Add a header "References" above the following fields in the inspector
        ...
        // Add a header "Icons" above the following fields in the inspector
        ...
        // Add a header "Other" above the following fields in the inspector
        [Header( "Other" )]
...
#pragma warning restore 0649
		...
		private void Awake()
		{
            // Awake() of FileBrowser class is executed in the process of executing:

            // Instantiate(Resources.Load<GameObject>("SimpleFileBrowserCanvas"))
            
            m_instance = this;           

			rectTransform = (RectTransform) transform; 
            // this.transform is the transform of the gameObject to which 
            // FileBrowser.cs component is attached.
             // This is an empty "root" gameObjects, which plays
             // the role of UI element, that is, file browser canvas.
             // note:
        ...
        // Start() is added by Moon Jung
        void Start()
        {

            //m_instance = Instantiate(Resources.Load<GameObject>("SimpleFileBrowserCanvas")).GetComponent<FileBrowser>();

            // change the properties of the file browser canvas to those of the canvas for the action
            // plan table:

            // m_actionPlanController = this.gameObject.GetComponent<ActionPlanController>();

            // m_browserCanvas = m_fileBrowserCanvasObj.GetComponent<Canvas>();

            //  m_browserCanvas.worldCamera = m_actionPlanController.m_canvas.worldCamera;

            // m_browserCanvas.renderMode = m_actionPlanController.m_canvas.renderMode;
            // already the same, though

            // m_browserCanvas.targetDisplay  = m_actionPlanController.m_canvas.targetDisplay;

            //m_browserCanvas.scaleFactor = m_actionPlanController.m_canvas.scaleFactor;


            // _browserCanvas.pixelRect = m_actionPlanController.m_canvas.pixelRect;

            // m_browserCanvas.planeDistance = m_actionPlanController.m_canvas.planeDistance;

            //RectTransform rect    = m_actionPlanController.m_canvas.GetComponent<RectTransform>(); // used in onGUI() uses display 1 (primary display)
            //m_fileBrowserCanvasObj.GetComponent<RectTransform>() = rect;
		...
		private void OnOperationSuccessful( string path )
		{
			...
			if( onSuccess != null )
				onSuccess( path ); // onSuccess refers to a lambda function: (path) => ();
										   ...
										   string title = "Save", string saveButtonText = "Save" )
		{
            ...
            //    private static FileBrowser Instance =? static field is a field of class
            //{
            //    get
            //    {
            //        if (m_instance == null)
            ...
            //Instance is a [static] singleton instance of FileBrowser
            // (1) Static members are initialized before the static  member is
            // accessed for the first time and before the static constructor, if
            // there is one, is called.
            //(2) If your class contains static fields, provide a static constructor
										   ...
										   string title = "Load", string loadButtonText = "Select" )
		{
			...
			Instance.onSuccess = onSuccess; // The input parameter onSuccess will set by lambda expression
                                              // (path) => (); This lambda function will be just registered by ShowLoadDialog()
                                               // will be executed later by some triggering events.
		...
		public static IEnumerator WaitForSaveDialog(OnSuccess onSuccess, OnCancel onCancel, bool folderMode = false, string initialPath = null,
													 string title = "Save", string saveButtonText = "Save" )
		{
			if( !ShowSaveDialog( onSuccess, onCancel, folderMode, initialPath, title, saveButtonText ) )
				yield break;
		...
		public static IEnumerator WaitForLoadDialog(OnSuccess onSuccess,  OnCancel onCancel, 
                                                     bool folderMode = false, string initialPath = null,
													 string title = "Load", string loadButtonText = "Select" )
		{
			if( !ShowLoadDialog( onSuccess, onCancel, folderMode, initialPath, title, loadButtonText ) )
				yield break;
