...
/// </remarks>
/// 
// Attached to ScrollView gameobject
public class MyScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    ...
    [SerializeField]
    private RectTransform m_ContentTimeLine;

    [SerializeField]
    private RectTransform m_ContentKeys;

    [SerializeField]
    private RectTransform m_ContentValues;

   

    public Vector2 m_ViewBoundsOffset;
        ...
        public RectTransform contentTimeLine { get { return m_ContentTimeLine; } set { m_ContentTimeLine = value; } }
        ...
        protected RectTransform viewRect
        {
            ...
            {
                if (m_ViewRect == null)
                    m_ViewRect = m_Viewport; // m_Viewport is specified in inspector
                if (m_ViewRect == null) // if m_Viewport is null
                    m_ViewRect = (RectTransform)transform; // get the transform attached to the ScrollView Rect
                return m_ViewRect;
            }
        }
        ...
        private RectTransform rectTransform
        {
            ...
            {
                if (m_Rect == null)
                    m_Rect = GetComponent<RectTransform>(); // The RectTransform attached to gameObject(ScrollView).
                return m_Rect;
            }
        }
        ...
        {  // move the   timeLine and the ContentValues horizontally

        RectTransform tempRect = content;
            // move m_ContentTimeLine according the horizontall scroll
            m_Content = contentTimeLine;
            SetNormalizedPosition(value, 0); // move m_Content

            // move m_ContentValues according to the horizontal scroll
        ...
        {
        // move the contentKeys and contentValues vertically
        // move the ContentKeys according to the vertical scroll
        RectTransform tempRect = content;

        content = contentKeys;
            SetNormalizedPosition(value, 1);

        // move the ContentValues according to the vertical scroll
    ...
    protected virtual void SetNormalizedPosition(float value, int axis)
        {
            EnsureLayoutHasRebuilt();
            UpdateBounds(); // get the bounds of the content panel, m_ContentBounds,  relative to the viewport
                            // How much the content is larger than the view.
                            // All the coordinates are set in the frame of the viewport
        float hiddenLength = m_ContentBounds.size[axis] - ( m_ViewBounds.size[axis] - m_ViewBoundsOffset[axis] );


        // Where the position of the lower left corner of the content bounds should be, in the space of the view.
        //float contentBoundsMinPosition = m_ViewBounds.min[axis] - value * hiddenLength;
        // The contentBoundsMinPosition should be  m_ViewBounds.min[axis] + m_ViewBoundsLeftOffset when value =0
        float contentBoundsMinPosition, contentBoundsMaxPosition;
        float newLocalPosition;

        if (axis == 0)
        { // horizontal direction: Left => Negative: Offset => Positive ; value =0: the leftmost 
            contentBoundsMinPosition = (m_ViewBounds.min[axis] + m_ViewBoundsOffset[axis]) - value * hiddenLength;
            // The new content localPosition, in the space of the view.
            newLocalPosition = m_Content.localPosition[axis] + (contentBoundsMinPosition - m_ContentBounds.min[axis]);
        } else // vertical direction: Down => Negative; Offset => Down => Negative; value =1: the topmost
        {
            contentBoundsMaxPosition = (m_ViewBounds.max[axis] - m_ViewBoundsOffset[axis]) + (1 - value) * hiddenLength;
            // The new content localPosition, in the space of the view.
            newLocalPosition = m_Content.localPosition[axis] + (contentBoundsMaxPosition - m_ContentBounds.max[axis]);
        }
        ...
        private Bounds GetBounds() // get the bounds of the content relative to the viewport
        {
            if (m_Content == null)
                return new Bounds();
